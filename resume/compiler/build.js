...
const DOCX_MODE = process.argv.includes('--docx');
...
async function main() {
  ...
  try {
    const { logBuildRun } = await import('../lib/spacetime-client.js');
    logBuildRun(Date.now(), candidateName, resume.work?.length ?? 0, resume.skills?.length ?? 0, 'success');
  } catch (err) {
    // Gracefully ignore if SpacetimeDB is not running
  }
}
...
main().catch(async (err) => {
  ...
  try {
    const { logBuildRun } = await import('../lib/spacetime-client.js');
    logBuildRun(Date.now(), 'Unknown', 0, 0, 'failure');
  } catch {}
  process.exit(1);
});
